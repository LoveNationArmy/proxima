...
(function () {
  ...
  const trim = html => html.trim().replace(/(>)(\s+)|(\s+)(<\/)/g, '$1$4');
  ...
  function parse (html, context = {}) {
    ...
    while (el.hasChildNodes()) fragment.appendChild(el.firstChild);
    ...
    while (tree.nextNode()) {
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        v = context.trim && !whitespaceTagNames.includes(node.parentNode.tagName)
          ? trim(node.textContent) : node.textContent;
      } else {
        v = { tag: node.tagName, ...attrs(node), children: [], context };

        for (const name in v.events) {
          const fn = new Function('event', 'context', `with (context) { ${v.events[name]} }`);
          v.events[name] = function (event) { return fn.call(this, event, v.context) };
        }
      }
  ...
  const attrs = node => [...node.attributes].reduce((p, n) =>
    (p[['attrs', 'events'][+(n.name.indexOf('on') === 0)]][n.name] = n.value, p), { attrs: {}, events: {} });

  const update = (node, v, parent, updated = v.updated) => {
    if (!v.tag) {
  ...
  const create = v => v.tag && (v.rendered = true)
  ...
  const dispatch = (node, event, fn = node.tagName && node[event]) => fn && fn.call(node);
  ...
  async function parse$1 (data, filter) {
  ...
  class Handlers {
    ...
    async join ({ cid, peer }) {
      if (peer.cid === cid) {
        ...
        peer.send(snapshot, await parse$1(snapshot));
      }
    }
  ...
  class Net extends EventTarget {
    ...
    async addPeer (peer) {
      ...
      for await (const { detail: data } of on(peer, 'data', 'close')) {
        ...
        if (chunk.size) {
          const view = await parse$1(chunk, msg => this.app.handlers.handle({ ...msg, peer }));

          if (view.data.size) {
            // console.log('broadcast:', view.data)
            this.broadcast(view.data, peer, await parse$1(new Set([...view.data, ...snapshot])));
          }
    ...
    async offerTo (cid) {
      if (this.peers.map(peer => peer.cid).includes(cid)) {
        ...
        this.peers.find(peer => peer.cid === cid).send([this.format('syncme')], await parse$1(this.app.state.merge(false, true)));
        return
      }
  ...
  class App {
    ...
    async dispatch (...message) {
      ...
      this.net.broadcast([message], this.net, await parse$1(this.state.merge(false, true)));
    ...
    async render () {
      this.state.view = await parse$1(this.state.merge(true));
